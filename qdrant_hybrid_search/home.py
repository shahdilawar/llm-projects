...
st.title(":blue[Welcome to Qdrant powered Hybrid Search] :sunglasses:")
...
if (len(search_text) != 0): 
    ...
    if not result_data_frame.empty:
        ...
        columns_to_display = ['rank', 'score', 'chunk_id', 'chunk_text', 'doc_url']
        subset_df = result_data_frame[columns_to_display]
            
        # Convert 'document_file' column to clickable URLs
        subset_df['doc_url'] = subset_df['doc_url'].apply(
    ...
    if not result_data_frame_1.empty:
       ...
       # Convert 'document_file' column to clickable URLs
       subset_df['doc_url'] = subset_df['doc_url'].apply(lambda x: f'<a href="{x}" target="_blank">{x}</a>')
